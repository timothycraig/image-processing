...
function displayFileSize(bytes, noColor = false) {
  ...
  if (noColor) return chalk.white(`${kb} KB`)
...
function calcPercentChange(oldSize, newSize) {
  ...
  try {
    switch (ext) {
      case '.png':
        ...
        if (calcPercentChange(origStats.size, processedStats.size) > 30 || wasResized) {
          console.log(`${displayFileSize(origStats.size)} -> ${displayFileSize(processedStats.size)} ${chalk.dim(`${calcPercentChange(origStats.size, processedStats.size)}%`)} ${chalk.underline(path.basename(outputPath))} ${chalk.italic(`(${processedMeta.width} X ${processedMeta.height})`)}`)
        } else {
          await fs.promises.cp(filePath, outputPath)

          console.log(`${displayFileSize(origStats.size)} -> ${displayFileSize(origStats.size, true)} ${chalk.dim('N/A')} ${chalk.underline(path.basename(outputPath))} ${chalk.italic(`(${processedMeta.width} X ${processedMeta.height})`)}`)
      ...
      default:
        ...
        console.log(`${displayFileSize(origStats.size, true)} -> ${displayFileSize(origStats.size, true)} ${chalk.dim('N/A')} ${chalk.underline(path.basename(outputPath))} ${chalk.italic(`(${processedMeta.width} X ${processedMeta.height})`)}`)
